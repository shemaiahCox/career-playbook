...
- **Eval JSONL complements unit tests**, it does **not** replace them: deterministic code paths (routing, preprocessing, serialization, BM25 boosts) still belong under normal automated tests ([Per-project testing — LLM/RAG layer](../playbook/per-project-testing.md)).
