...
- [ ] **Mutable default arguments** (`def f(x=[]):`) and **shared mutable state** across requests—classic bug class. **Fix:** `def f(x=None): x = [] if x is None else x`. Deep dive: [Gotchas #2, #6, #9, #18](language-gotchas-deep-dive.md).
